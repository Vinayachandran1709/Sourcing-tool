        ...
        try:
            ...
            response = await client.get(
                ...
                params={
                    "sort": "stars",
                    "direction": "desc",
                    "per_page": max_repos,  # Should be 100 by default
                    "type": "owner"
                },
...
def extract_top_repos(repos, top_n: int = 5):
    ...
    try:
        ...
        print("Error processing repos:", e)
...
async def calculate_language_distribution(username: str, repos, max_repos_to_check: int = 20):
    ...
    for language, bytes_count in total_bytes.items():
        percentage = round((bytes_count / total) * 100, 1)  # Round to 1 decimal
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response = await client.get(
                ...
                headers=headers,
                params={"per_page": 100}  # Get last 100 events
            )
